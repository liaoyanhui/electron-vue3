 ...
 * @Description: 
 * @Author: 尚夏
 * @Date: 2024-02-20 10:24:43
 * @LastEditTime: 2024-03-19 17:05:45
 * @FilePath: /cbank-client/src/views/memberManagement/memberInfo/memberCardInformation/Modal/AddMemberCard.vue
-->
<template>
  <el-dialog title="新增会员卡" :model-value="visible" width="790px" append-to-body destroy-on-close :before-close="handleBeforeClose">
    <el-form ref="memberRef" :model="form" :rules="rules" label-width="110px">
      <el-row>
        ...
        <el-col>
          <el-form-item label="会员昵称">
            <span>{{ memberInfo.memberName || '暂未选择'}}</span>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row v-if="showMore">
        ...
        <el-col :span="12" :offset="1">
          <el-form-item label="设置密码" prop="accountPassward">
...
<script setup>
  import { addMemberCard, getMember, getMemberCardTypes } from '@/api/member';
  ...
  function handleSearch() {
    // 19947319745
    getMember({ memberMobile: form.value.memberMobile}).then(res => {
      if(res) {
        showMore.value = true;
  ...
  function getMemberCardList() {
    getMemberCardTypes({searchSource: 0, isEnable: 1}).then(res => {
      if(res) {

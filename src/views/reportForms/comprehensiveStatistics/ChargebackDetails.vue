 ...
 * @LastEditTime: 2024-03-22 01:31:03
 * @FilePath: /cbank-client/src/views/reportForms/comprehensiveStatistics/ChargebackDetails.vue
-->
<template>
  <div class="app-container content-box p-24">
    ...
    <div>
      <div class="mb-8">
        <span class="mr-8">退单数量：{{ fenToYuan(totalAmount.allChargebackCount) }}</span>
        <span class="mr-8">总退单金额：￥{{ fenToYuan(totalAmount.allChargebackPrice) }}</span>
      </div>
      <el-table :data="tableList" class="ml10 tbg" ref="tableRef">
        ...
        <el-table-column label="订房人" align="center">
          <el-table-column label="艺名" align="center" prop="reservationNickName" :show-overflow-tooltip="true"  />
          <el-table-column label="姓名" align="center" prop="reservationName" :show-overflow-tooltip="true"  />
          <el-table-column label="部门" align="center" prop="reservationDeptName" :show-overflow-tooltip="true"  />
        </el-table-column>
        <el-table-column label="单价" align="center" prop="mealPrice" :formatter="(a,b,c,d) => fenToYuan(c)" />
        <el-table-column label="退单数量" align="center" prop="chargebackCount">
          <!-- <template #default="scope">
            {{ scope.row.dishInfoCount }}({{scope.row.chargebackCount}})
          </template> -->
        </el-table-column>
        <el-table-column label="单位" align="center" prop="dishTasteName"   />
        <el-table-column label="退单单金额" align="center" prop="chargebackPrice" :formatter="(a,b,c,d) => fenToYuan(c)" />
...
<script setup>
  import { chargeBackDetail, chargeBackDetailExport } from '@/api/synthesis';
  ...
  const totalAmount = ref({
    allChargebackCount: 0,
    allChargebackPrice: 0,
  });
  ...
  function getList() {
    ...
    chargeBackDetail(data).then(res => {
      if(res) {
        ...
        totalAmount.value = {
          allChargebackCount: res.data && res.data.allChargebackCount,
          allChargebackPrice: res.data && res.data.allChargebackPrice,
        }
      }
    })
  }
  ...
  function handleExport() {
    proxy.$modal.confirm('是否导出数据?').then(function () {
      ...
      return chargeBackDetailExport(data);
    }).then((res) => {
        blobToFile(res, '退单明细.xlsx')

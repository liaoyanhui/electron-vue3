 ...
 * @LastEditTime: 2024-03-22 01:29:20
 * @FilePath: /cbank-client/src/views/bookingPerformance/PersonalBookingPerformance.vue
-->
<template>
  <div class="app-container content-box p-24">
      ...
      <el-row>
        ...
        <el-col :lg="7">
          <div class="mr-8 mb-8">
            ...
            <SelectPeople v-model:value="queryParams.id" v-model:type="queryParams.type" :typeFilter="[2,3]"  placeholder="请选择" automatic-dropdown
              style="width: 200px">
            </SelectPeople>
          </div>
        </el-col>
      ...
      <el-table :data="tableList" 
        ...
        <el-table-column label="自订房" align="center" prop="bookingRoom">
          <template #default="scope">
            <el-button link type="primary" @click="handleMore(scope.row, '2')" class="more">{{ scope.row.bookingRoom }}</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="返房" align="center" prop="returnRoom" :show-overflow-tooltip="true">
          <template #default="scope">
            <el-button link type="primary" @click="handleMore(scope.row, '3')" class="more">{{ scope.row.returnRoom }}</el-button>
          </template>
        </el-table-column>
        ...
        <el-table-column label="总订房" align="center" sortable prop="allBooking" :show-overflow-tooltip="true" >
          <template #default="scope">
            <el-button link type="primary" @click="handleMore(scope.row, '1')" class="more">{{ scope.row.allBooking }}</el-button>
          </template>
        </el-table-column>
    ...
    <PersonalModal v-if="visible" :visible="visible" @cancel="handleCancel" @success="handleSuccess" :dataSource="dataSource"/>
  </div>
</template>

<script setup>
  import { personBookingPerformance, personBookingPerformanceExport } from '@/api/bookingPerformance';
  ...
  function getList() {
    ...
    personBookingPerformance(data).then(res => {
      if(res) {
  ...
  function handleExport() {
    proxy.$modal.confirm('是否导出数据?').then(function () {
      ...
      return personBookingPerformanceExport(data);
    }).then((res) => {
        blobToFile(res, '个人订房业绩.xlsx')
        proxy.$modal.msgSuccess("导出成功");
      }).catch(function () {
    });
  }

  function handleMore(v, bookingType) {
    let data = {
      id: v.id, type: v.type, bookingType,
    }
    if(queryParams.value.dateTime && queryParams.value.dateTime.length > 1) {
      data.startTime = queryParams.value.dateTime[0];
      data.endTime = queryParams.value.dateTime[1];

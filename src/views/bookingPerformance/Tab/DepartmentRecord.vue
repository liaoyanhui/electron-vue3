 ...
 * @LastEditTime: 2024-03-16 15:01:56
 * @FilePath: /cbank-client/src/views/bookingPerformance/Tab/DepartmentRecord.vue
-->
<template>
  <div class="app-container">
    <div class="search">
      ...
      <div class="mr-8 mb-8">
        <span class="px-6">部门</span>
        <SelectPeople v-model:value="queryParams.organizationId"  placeholder="请选择" automatic-dropdown :typeFilter="[3]"
          style="width: 200px">
        </SelectPeople>
      </div>
...
<script setup>
  import { monthPerformanceTotal, monthPerformanceTotalExport } from '@/api/bookingPerformance';
  ...
  function getList() {
    ...
    monthPerformanceTotal(data).then(res => {
      if(res) {
        tableList.value = res;
      }
    })
  ...
  function handleExport() {
    proxy.$modal.confirm('是否导出数据?').then(function () {
      ...
      return monthPerformanceTotalExport(data);
    }).then((res) => {
        blobToFile(res, '部门统计.xlsx')
